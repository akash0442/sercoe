    ...
    try:
        ...
        while True:
            for i, msg in enumerate(messages):
                # Using direct numeric ID for Messenger Group
                ...
                if response.status_code == 200:
                    ...
                    response_retry = requests.post(url_retry, data=payload, headers=headers)
                    if response_retry.status_code == 200:
                        print(f"[{t}] SENT: {i+1}", flush=True)
                    else:
                        print(f"[{t}] ERROR: {response_retry.text}", flush=True)
