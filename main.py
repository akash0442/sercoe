...
class MyHandler(http.server.SimpleHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"SAFE BOT IS RUNNING")
...
def send_messages():
    print(">> STARTING STEALTH BOT (SAFE MODE) <<", flush=True)
    try:
        with open('time.txt', 'r') as f: base_delay = int(f.read().strip())
        if base_delay < 60: base_delay = 60 # Force minimum 1 min gap
        ...
        while True:
            for i, msg in enumerate(messages):
                ...
                if response.status_code == 200:
                    print(f"[{t}] MSG {i+1} DISPATCHED. STATUS: OK", flush=True)
                else:
                    print(f"[{t}] DISPATCH FAILED: {response.status_code}", flush=True)
                
                # Randomized delay to bypass bot detection
                sleep_time = base_delay + random.randint(5, 20)
                print(f"Waiting {sleep_time}s for next message...", flush=True)
